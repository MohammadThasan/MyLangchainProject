...
#OPENAI_API_KEY="" # If you want to reveal your API keys direct here which is not advisable for security
...
#---------------------------------------------#
# Step 5: Conditional Logic and AI Invocation #
#---------------------------------------------#

#💡 Explanation:
# if city and month and language and budget: Checks that ALL inputs have values before proceeding. This prevents errors from missing data.
# prompt_template.format(): Fills in the template placeholders with actual user values.
# llm.invoke(): Sends the formatted prompt to the AI model and waits for a response.
# response.content: Extracts the AI's text response from the response object.
# st.write(): Displays the AI's response on the web page.
# This only runs when users have filled all the fields!
